...
// NOTE: In production, proxy this.
const SERP_API_BASE = "https://serpapi.com/search.json";

/**
 * Normalizes specific Google News JSON structure to our flat NewsArticle format.
 */
const normalizeSerpResponse = (data: any, keyword: string): NewsArticle[] => {
  ...
  items.forEach((item: any, index: number) => {
    // 1. Handle "Stories" (clusters returned by Google)
    if (item.stories && Array.isArray(item.stories)) {
      item.stories.forEach((story: any, sIndex: number) => {
        normalized.push({
          article_id: "", // Will be hashed later
          title: story.title,
          url: story.link,
          source_name: story.source?.name || story.source || "Desconocido",
          published_at: story.date || new Date().toISOString(), // SerpApi returns relative "2 hours ago" often, real engine returns ISO sometimes. logic needs to handle strings.
          thumbnail_url: item.thumbnail || undefined, // Often the parent has the thumb
          position: index + 1,
          keyword: keyword,
    ...
    // 2. Handle Single Articles
    // Note: Sometimes an item has both 'stories' and is an article itself. We add the main one too.
    normalized.push({
      article_id: "", // Will be hashed later
      title: item.title,
      url: item.link,
...
export const fetchSerpResults = async (settings: AppSettings): Promise<NewsArticle[]> => {
  // 1. If no key is provided, return mocks immediately
  ...
  const fetchQuery = async (query: string): Promise<NewsArticle[]> => {
    ...
    url.searchParams.append("engine", "google_news"); // UPDATED ENGINE
    url.searchParams.append("q", query);
    url.searchParams.append("api_key", settings.serpApiKey);
    url.searchParams.append("gl", "es"); // Geo Location: Spain
    url.searchParams.append("hl", "es"); // Host Language: Spanish
    url.searchParams.append("num", "100"); // UPDATED: Fetch max allowed to get a large list

    try {
      // NOTE: Direct calls to SerpApi from browser often fail due to CORS. 
  ...
  // Run queries sequentially or with limited concurrency to avoid hitting rate limits too hard if many queries
  const promises = settings.searchQueries.map(q => fetchQuery(q));
  const resultsArray = await Promise.all(promises);
  resultsArray.forEach(res => allArticles.push(...res));

  // 2. FALLBACK STRATEGY
  // If we attempted to fetch but got errors (likely CORS) and ended up with empty results,
  // return the mock data so the user experience isn't broken.
...
// Expanded Mock data to simulate a "Large List"
